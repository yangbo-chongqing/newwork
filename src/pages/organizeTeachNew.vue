    ...
    <div id="nav">
      <div class="typeArea">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item :to="{ path: '/organizeTeach.html' }">德智教育</el-breadcrumb-item>
          <el-breadcrumb-item :to="{ path: '' }">文档学习</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
    </div>
    <div class="typeArea">
      <h2>{{ news.classTitle }}</h2>
      <h4>{{ news.create_date | formatTime}} <!--阅读次数：<span>{{ news.read_count}}</span>--></h4>
      <h6>发布人：<span>{{news.userName}}</span></h6>
      <div id="info" v-html="news.classContent"></div>
...
<script>
    export default {
        ...
        methods: {
            getMsg(){
                this.$axios.get('/fun/class/info',{
                    params:{
                        classId:this.news_id
                    }
                }).then(msg=>{
                    console.log(msg);
                    if(msg.status == 200){
                        this.news = msg.data.data
                    }else{
